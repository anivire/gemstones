...
public class ModelProvider extends FabricModelProvider {
  ...
  public void generateItemModels(ItemModelGenerator itemModelGenerator) {
    for (Item item : ItemsRegistry.getAllItems()) {
      if (item instanceof BlockItem) {
        if (item == BlocksRegistry.jewelryTable().asItem()) {
      ...
      String baseName = path.replaceAll("_(crude|refined|polished|flawless|radiant|mythic|unusual|amplifier)$", "");
  ...
  private boolean isHandheldTool(Item item) {
    return item == ItemsRegistry.diamondTippedChisel()
        || item == ItemsRegistry.netheriteTippedChisel()
        || item == ItemsRegistry.jewelryFile()
        || item == ItemsRegistry.jewelryPliers()
        || item == ItemsRegistry.jewelryHammer();
  }
}
