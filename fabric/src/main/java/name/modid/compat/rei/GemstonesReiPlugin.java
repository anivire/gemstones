...
public class GemstonesReiPlugin implements REIClientPlugin {
  ...
  public void registerCategories(CategoryRegistry registry) {
    ...
    registry.addWorkstations(INSERT, EntryStacks.of(BlocksRegistry.jewelryTable()));
    registry.addWorkstations(REMOVE, EntryStacks.of(BlocksRegistry.jewelryTable()));
    registry.addWorkstations(EXPAND, EntryStacks.of(BlocksRegistry.jewelryTable()));
  }
  ...
  private static List<JewelryTableReiDisplay> insertDisplays() {
    ...
    for (ItemStack gem : JewelryTableRecipeExamples.allGemstoneStacks()) {
      ...
      displays.add(new JewelryTableReiDisplay(INSERT,
          List.of(
              ingredient(ItemsRegistry.jewelryHammer()),
              baseIngredient,
              ingredient(gem)),
  ...
  private static List<JewelryTableReiDisplay> removeDisplays() {
    ...
    for (ItemStack gem : JewelryTableRecipeExamples.allGemstoneStacks()) {
      displays.add(new JewelryTableReiDisplay(REMOVE,
          List.of(
              ingredient(ItemsRegistry.jewelryPliers()),
  ...
  private static List<JewelryTableReiDisplay> expandDisplays() {
    ...
    return List.of(new JewelryTableReiDisplay(EXPAND,
        List.of(
            ingredient(List.of(
                new ItemStack(ItemsRegistry.diamondTippedChisel()),
                new ItemStack(ItemsRegistry.netheriteTippedChisel()))),
            baseIngredient,
            ingredient(ItemsRegistry.expansionCrystal())),
        List.of(resultIngredient),
        false));
  }
